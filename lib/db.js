...
// Find the Postgres connection string, tolerating integration prefixes
// (e.g. Vercel/Neon may inject LAYR_POSTGRES_URL instead of POSTGRES_URL).
function connectionString() {
  const direct =
    process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_PRISMA_URL ||
    process.env.LAYR_DATABASE_URL || process.env.LAYR_POSTGRES_URL || process.env.LAYR_POSTGRES_PRISMA_URL;
  if (direct) return direct;
  // Fallback: any env var ending in DATABASE_URL / POSTGRES_URL that looks like a pg URL.
  for (const [k, v] of Object.entries(process.env)) {
    if (/(^|_)(DATABASE_URL|POSTGRES_URL)$/.test(k) && typeof v === 'string' && /^postgres(ql)?:\/\//.test(v)) return v;
  }
  return null;
}
...
function client() {
  if (!_client) {
    ...
    if (!cs) throw new Error('No database connection string. Set DATABASE_URL or POSTGRES_URL (any prefix).');
    _client = neon(cs);
  }
  return _client;
}
